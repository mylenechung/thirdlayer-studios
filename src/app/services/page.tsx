...
export default function ServicesPage() {
  ...
  // Consistent section padding across all three breakpoints
  ...
  return (
    <div style={{ background: C.beige, minHeight: '100vh' }}>
      ...
      {/* How It Works */}
      <section style={{ padding: `${P.split(' ')[0]} ${P.split(' ')[1] ?? P.split(' ')[0]}`, background: C.dark }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: mob ? 'repeat(2,1fr)' : tab ? 'repeat(2,1fr)' : 'repeat(4,1fr)', gap: mob ? 24 : tab ? 28 : 32, marginBottom: mob ? 40 : 64 }}>
          {steps.map(({ s, t, d }) => (
            <div key={s} style={{ borderTop: '1px solid rgba(243,239,232,0.12)', paddingTop: 24 }}>
              ...
              <h4 style={{ fontFamily: 'var(--font-righteous),cursive', fontSize: mob ? 17 : 20, color: C.beige, margin: '0 0 12px' }}>{t}</h4>
